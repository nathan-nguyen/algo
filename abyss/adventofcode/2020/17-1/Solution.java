...
public class Solution {
    ...
    private void add(Set<Long> set, int x, int y, int z) {
        set.add(getKey(x, y, z));
    }

    private long getKey(int x, int y, int z) {
        return x * 1000_000_000_000L + y * 1000_000L + z;
    }
    ...
    private List<Long> getNeighborList(long p) {
        ...
        int z = (int) (p % 1000_000L);
        int x = (int) (p / 1000_000_000_000L);
        int y = (int) ((p - x * 1000_000_000_000L - z) / 1000_000L);
        for (int i = -1; i <= 1; ++i) {
            for (int j = -1; j <= 1; ++j) {
                for (int k = -1; k <= 1; ++k) {
                    if (i == 0 && j == 0 && k == 0) continue;
                    long nb = getKey(x + i, y + j, z + k);
                    nbl.add(nb);
                }
            }
        }
